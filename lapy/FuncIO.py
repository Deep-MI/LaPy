    ...
    while i < len(ll):
        if ll[i].lstrip().startswith('Creator:'):
            ...
            while not (ll[i].strip().startswith("sizes")):
                i = i + 1
            d.update({'EigenvectorsSize': np.array(ll[i].strip().split()[1:]).astype(np.int)})
            i = i + 1
            while ll[i].find("{") < 0:  # possibly introduce termination criterion
                i = i + 1
            if ll[i].find("}") >= 0:  # '{' and '}' on the same line
                ...
                evecs = str()
                while ll[i].find("}") < 0:
                    evecs = evecs + ll[i].strip().replace('{', '').replace('}', '').replace('(', '').replace(')', '')
                    i = i + 1
                evecs = evecs + ll[i].strip().replace('{', '').replace('}', '').replace('(', '').replace(')', '')
            evecs = np.array(evecs.replace(';', ' ').replace(',', ' ').strip().split()).astype(np.float)
            if len(evecs) == (d['EigenvectorsSize'][0] * d['EigenvectorsSize'][1]):
                ...
                print('[Length of eigenvectors is not ' + str(d['EigenvectorsSize'][0]) + ' times ' + str(
                    d['EigenvectorsSize'][1]) + '.')
            i = i + 1
        else:
            i = i + 1
    # close file
