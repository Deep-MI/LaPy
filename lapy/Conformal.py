...
def spherical_conformal_map(tria: TriaMesh) -> np.ndarray:
    """
    Computes a spherical conformal map of a genus-0 closed surface.

    This function produces a conformal parameterization of a genus-0 closed surface
    onto a sphere using a linear method. It involves solving the Laplace equation on a
    carefully selected "big triangle" and further optimizing the mapping via
    inverse stereographic projection and area adjustment.
    ...
    References:
        ...
        Adopted from the Matlab implementation by Gary Pui-Tung Choi:
        https://github.com/garyptchoi/spherical-conformal-map

    Parameters:
        tria (TriaMesh):
            A triangular mesh object representing a genus-0 closed surface with attributes:
            - `v`: A NumPy array of shape (n, 3) containing vertex coordinates.
            - `t`: A NumPy array of shape (m, 3) containing triangle indices.
            - `euler()`: A method to compute the Euler characteristic (used to verify genus-0 topology).
            - `tria_qualities()`: A method to compute the quality of triangles (used to select the "big triangle").
            - `v[p1] - v[p0]`: Vertex arithmetic to construct local coordinates for boundary conditions.

    Returns:
        np.ndarray:
            A NumPy array of shape (n, 3) representing the spherical conformal parameterization of vertices.
    """
    ...
    idx = np.argsort(S[:, 2])
    fixnum = max(round(nv / 10), 3)  # Fix 1/10th of the vertices
    fixed = idx[:min(nv, fixnum)]

    # South pole stereographic projection
    P = np.column_stack((S[:, 0] / (1 + S[:, 2]), S[:, 1] / (1 + S[:, 2]), np.zeros(nv)))

    # Compute Beltrami coefficients for the current parameterization
    triasouth = TriaMesh(P, tria.t)
    mu = beltrami_coefficient(triasouth, tria.v)

    # Solve for a quasi-conformal map that cancels distortion
...
def mobius_area_correction_spherical(tria: TriaMesh, mapping: np.ndarray) -> Tuple[np.ndarray, Any]:
    """
    Finds an optimal Möbius transformation for reducing the area distortion of a spherical conformal parameterization.

    This method minimizes the area distortion using the approach described in:
    ...
    References:
        Adopted by Martin Reuter from Matlab code by Gary Pui-Tung Choi:
        https://github.com/garyptchoi/spherical-conformal-map

    Parameters:
        tria (TriaMesh):
            A triangular mesh object of genus-0 closed surfaces with attributes:
            - `v`: A NumPy array of shape (n, 3) containing the 3D vertex coordinates.
            - `t`: A NumPy array of shape (m, 3) containing triangle indices.
            - `tria_areas()`: A method to compute the areas of triangles in the mesh.
        mapping (np.ndarray):
    ...
    Returns:
        Tuple[np.ndarray, Any]:
            A tuple containing:
            - `map_mobius`: A NumPy array of shape (n, 3) with vertex coordinates updated to minimize area distortion.
            - `result`: The optimization result object from `scipy.optimize.minimize`.
    """
...
def beltrami_coefficient(tria: TriaMesh, mapping: np.ndarray) -> np.ndarray:
    ...
    References:
        ...
        Original code adopted from Matlab implementation by Gary Pui-Tung Choi:
        https://github.com/garyptchoi/spherical-conformal-map

    Parameters:
        tria (TriaMesh):
            A triangulation object with attributes:
            - `v`: A numpy array of shape (n, 3) containing 3D coordinates of vertices.
            - `t`: A numpy array of shape (m, 3) containing indices of vertices forming triangles.
        mapping (np.ndarray):
            A numpy array of shape (n, 3) representing the mapped coordinates of vertices.

    Returns:
        np.ndarray:
            A complex-valued numpy array of shape (m,) representing the Beltrami coefficient
    ...
    tids = np.arange(nf)
    i = np.column_stack((tids, tids, tids)).reshape(-1)  # Triangle indices
    j = tria.t.reshape(-1)  # Vertex indices
...
def linear_beltrami_solver(
    ...
    Parameters:
        tria (TriaMesh):
            A triangulation object with attributes:
            - `v`: A numpy array of shape (n, 3) containing triangle vertices in 3D coordinates.
            - `t`: A numpy array of shape (m, 3) containing triangle indices.
        mu (np.ndarray):
            A numpy array containing Beltrami coefficients, describing distortion at each vertex.
        landmark (np.ndarray):
            A numpy array of indices specifying the landmark vertices.
        target (np.ndarray):
            A numpy array of shape (len(landmark), 2), specifying the target positions
...
def sparse_symmetric_solve(A: csr_matrix, b: Union[np.ndarray, csr_matrix], use_cholmod: bool = True) -> np.ndarray:
    """
    ...
    Parameters:
        ...
        use_cholmod (bool, optional):
            If True, attempts to use Cholesky decomposition (default: True).
    ...
    if use_cholmod:
        try:
            ...
            use_cholmod = False
    if use_cholmod:
        print("Solver: Cholesky decomposition - performance optimal ...")
        chol = cholesky(A)
        x: np.ndarray = chol(b)
    else:
        print("Package scikit-sparse not found (Cholesky decomposition)")
        print("Solver: LU decomposition (spsolve) - performance not optimal ...")
        lu = splu(A)
        x: np.ndarray = lu.solve(b)
    return x    
